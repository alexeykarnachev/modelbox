...
ImageToDepthMaskModelName = Literal["depth_pro"]


def _transform_image_to_depth_mask_depthpro(image: Image.Image) -> Image.Image:
...
def transform_image_to_depth_mask(
    image: Image.Image, model_name: ImageToDepthMaskModelName
) -> Image.Image:
    if model_name == "depth_pro":
        out_image = _transform_image_to_depth_mask_depthpro(image)
